...
int IridiumSBD::internalSendReceiveSBD(const char *txTxtMessage, const uint8_t *txData, size_t txDataSize, uint8_t *rxBuffer, size_t *prxBufferSize)
{
   ...
   if (txData && txDataSize)
   {
      ...
      uint16_t checksum = 0;
      for (uint16_t i=0; i<txDataSize; ++i)  //ARDUNIO IDE ERROR FIXES: int i changed to uint16_t
      {
      ...
      console(F("["));
      console((uint16_t)txDataSize); //ARDUNIO IDE ERROR FIXES: added explicit cast to (uint16_t)
      console(F(" bytes]"));
   ...
   else // Text transmission
   {
      ...
      if (txTxtMessage) // It's ok to have a NULL txTxtMessage if the transaction is RX only
         send(txTxtMessage);
      send(F("\r"), false);
   ...
   for (unsigned long start = millis(); millis() - start < 1000UL * ISBD_DEFAULT_SENDRECEIVE_TIME;)
   {
	  ...
	  if (useWorkaround && strength >= minimumCSQ)
      {
		 ...
		 delay(100); // MSSTM always fails on first run, seeing if giving RB more time helps
...
bool IridiumSBD::waitForATResponse(char *response, int responseSize, const char *prompt, const char *terminator)
{
   ...
   //bool done = false; //ARDUNIO IDE ERROR FIXES: unused variable commented out
...
int IridiumSBD::doSBDRB(uint8_t *rxBuffer, size_t *prxBufferSize)
{
   ...
   delay(100);//timing error? Added to try to receive all
   
   for (int i=0; i<size; i++){//Logic error? Switched to i++
      if (cancelled()){
		return ISBD_CANCELLED;
	  }
      ...
      if (stream.available()){
         ...
         if (rxBuffer && prxBufferSize)//ARDUNIO IDE ERROR FIXES: Wrapping braces added
	     {
            if (*prxBufferSize > 0)
            {
               *rxBuffer++ = c;
			   console((char)c);//added for debug
               (*prxBufferSize)--;
            }
            else
            {
